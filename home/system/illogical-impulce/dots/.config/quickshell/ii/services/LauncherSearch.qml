...
Singleton {
    ...
    function ensurePrefix(prefix) {
        if ([Config.options.search.prefix.action, Config.options.search.prefix.app, Config.options.search.prefix.clipboard, Config.options.search.prefix.math, Config.options.search.prefix.shellCommand, Config.options.search.prefix.webSearch,].some(i => root.query.startsWith(i))) {
    ...
    property list<string> appCategories: Array.from(
        DesktopEntries.applications.values.reduce((acc, entry) => {
            for (const cat of entry.categories)
                if (mainRegisteredCategories.includes(cat)) acc.add(cat);
            return acc;
        }, new Set())
    ).sort()
    ...
    property list<var> results: {
        ...
        if (root.query.startsWith(Config.options.search.prefix.clipboard)) {
            ...
            return Cliphist.fuzzyQuery(searchString).slice(0, Config.options.search.maxResults ?? 10).map((entry, index, array) => {
            ...
            return resultComp.createObject(null, {
                ...
                execute: () => {
                    if (!entry.runInTerminal)
                        ...
                        Quickshell.execDetached(["sh", '-c', `${Config.options.apps.terminal} '${StringUtils.shellSingleQuoteEscape(entry.command.join(' '))}'`]);
                    }
                },
                    ...
                    return resultComp.createObject(null, {
                        ...
                        execute: () => {
                            if (!action.runInTerminal)
                                ...
                                Quickshell.execDetached(["sh", '-c', `${Config.options.apps.terminal} '${StringUtils.shellSingleQuoteEscape(action.command.join(' '))}'`]);
                            }
                        }
                    });
                })
        ...
        const commandResultObject = (startsWithShellCommandPrefix || Config.options.search.prefix.showDefaultActionsWithoutPrefix)
        ? resultComp.createObject(null, {
            ...
            execute: () => {
                ...
                Quickshell.execDetached(["sh", "-c", root.query.startsWith('sudo') ? `${Config.options.apps.terminal} '${cleanedCommand}'` : cleanedCommand]);
            }
        }) : null;

        const mathResultObject = (startsWithNumber || startsWithMathPrefix || Config.options.search.prefix.showDefaultActionsWithoutPrefix)
        ? resultComp.createObject(null, {
        ...
        if (Config.options.search.prefix.showDefaultActionsWithoutPrefix) {
            if (!startsWithNumber && !startsWithShellCommandPrefix)
                result.push(commandResultObject);
            if (!startsWithNumber && !startsWithMathPrefix)
                result.push(mathResultObject);
        }

        if (!startsWithNumber && !startsWithMathPrefix && !startsWithShellCommandPrefix && !startsWithWebSearchPrefix)
            result.push(webSearchResultObject);
    ...
    onQueryChanged: {
    if (!query.startsWith(Config.options.search.prefix.clipboard))
